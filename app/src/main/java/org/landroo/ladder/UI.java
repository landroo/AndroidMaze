...
public class UI 
{
    ...
    public boolean tapEvent(MotionEvent event)
    {
    	...
    	for(int id = 0; id < event.getPointerCount(); id++)
    	{
			...
			fingers[id].mode = event.getAction();
			fingers[id].idx = (fingers[id].mode & MotionEvent.ACTION_POINTER_INDEX_MASK) >> MotionEvent.ACTION_POINTER_INDEX_SHIFT;
			fingers[id].action = fingers[id].mode & MotionEvent.ACTION_MASK;
			fingers[id].x = x;
			fingers[id].y = y;
    	}
